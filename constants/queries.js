...
queries.get_order_prods=function(id_cliente, id_pedido){return "SELECT * from tb_pedido LEFT JOIN (SELECT * from item_pedido left join tb_produto using(id_prod)) as sub using(id_pedido) LEFT JOIN tb_pagamento USING(id_pagamento) where id_cliente = "+id_cliente+" AND id_pedido = "+id_pedido;}
