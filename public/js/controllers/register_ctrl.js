...
angular.module('systennis')
	.controller('register_ctrl', function($scope, $state, $http){
		...
		$scope.check_where_am_i = function() {
            ...
            if ($state.params.addressData) {page ++;}
            if ($state.params.registrationStatus) {page = 3;}
		...
		$scope.cadastrar = function (cadastro) {
			...
			.success(function(response){
				...
				else
				{
					...
					$state.go('register_2', {userData: response});
				};
			});
		};

		$scope.cadastrar_end = function (cadastro) {
			...
			.success(function(response){
                ...
                else
                {
                    ...
                    var userData = $state.params.userData;
                    var addressData = response;

                    $state.go('register_3', {userData: userData, addressData: addressData});
                }
			});
		};
        ...
        $scope.confirmar_userdata = function () {
            ...
            $http.post('/users/register_3', regParams)
            .success(function(response){
        ...
        $scope.formatAndShowUserData = function (){
            ...
            $scope.tbfUserData =
            {"(1) Nome Completo": ($scope.userData.nome_cliente + " " + $scope.userData.sobrenome_cliente),
            "(2) E-Mail": ($scope.userData.email_cliente),
            "(3) CPF": ($scope.userData.cpf_cliente),
		...
		$scope.setActiveStep = function(step) {
			if (step > 1)
			{
				$scope.activeBarTwo = 'active';
			}
			if (step > 2)
			{
				$scope.activeBarThree = 'active';
			}
			if (step > 3)
			{
				$scope.activeBarFour = 'active';
			}
		};

		$scope.fetch_cities = function (state) {

			$scope.selected_state = state;

			function filterByState(state) {
			  return state.sigla === $scope.selected_state;
			};

			$scope.cities = ($scope.lista_estados.estados.find(filterByState)).cidades;
		};

		$scope.fetch_zipcode_details = function (zipcode) {

			$scope.selected_zipcode = zipcode;

			var base_url = 'https://viacep.com.br/ws/';
			var url = base_url + zipcode + '/json'
			...
			$http.get(url).success(function(response)
			{
				...
				else
				{
					...
					var stateCandidateIndex = $scope.estados.indexOf($scope.address.state);

					if (stateCandidateIndex >= 0)
					{
						$('#state').val(stateCandidateIndex);

						// console.log($scope.address.city);

					    $scope.fetch_cities($scope.address.state);

					    var cityCandidateIndex = $scope.cities.indexOf(response.localidade)

						if(cityCandidateIndex >= 0)
						{
                            $scope.address.city = response.localidade;
							// $('#city').val(cityCandidateIndex);
						}
					}
				}

				// $('#rua').val($scope.address.street);
				// $('#bairro').val($scope.address.bairro);
